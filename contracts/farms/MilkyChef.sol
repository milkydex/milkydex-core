...
contract MilkyChef is Ownable {
    ...
    IMultiFeeDistribution public midRewarder;
    ...
    function massUpdatePools(uint256[] memory pids) public {
        require(pids.length > 0, "Require at least 1 pool");
    ...
    function harvest(uint256 pid, address to) public {
        ...
        // Interactions
        if (_pendingReward != 0) {
            midRewarder.mint(to, _pendingReward);
        }
    ...
    function harvestMultiplePools(address to, uint256[] memory pids) external {
    ...
    /// @param _update True: will update all pools (might reach gas limit).
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        IRewarder _rewarder,
    ...
    /// @notice Sets the reward per second to be distributed. Can only be called by the owner or emission admin.
    /// @param _rewardPerSecond The amount of reward to be distributed per second.
    function setRewardPerSecond(uint256 _rewardPerSecond, bool _update) public {
        require(
            msg.sender == owner() || msg.sender == emissionAdmin,
            "MilkyChef:: setRewardPerSecond: NOT ALLOWED"
        );
        if (_update) {
            massUpdateAllPools();
        }
    ...
    /// @notice Set the address of midRewarder
    /// @param _midRewarder Address of MultiFeeDistribution contract
    function setMidRewarder(address _midRewarder) external onlyOwner {
        require(_midRewarder != address(0), "MilkyChef:: INVALID ADDRESS");
        midRewarder = IMultiFeeDistribution(_midRewarder);
        emit MidRewarderUpdated(_midRewarder);
    }

    function setEmissionAdmin(address _admin) external onlyOwner {
        require(_admin != address(0), "MilkyChef:: INVALID ADDRESS");
        emissionAdmin = _admin;
        emit EmissionAdminSet(_admin);
    }
    ...
    event MidRewarderUpdated(address indexed _midRewarder);
    event EmissionAdminSet(address indexed _admin);
}
